...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while k <= num_rolls:
        curr_dice = dice()
        if curr_dice == 1: # Pig out!
            # Roll the rest turn
            for i in range(num_rolls-k): dice()
            return 1
        else:
            total += curr_dice
        k = k + 1
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    strategys = [strategy0, strategy1]
    scores = [score0, score1]
    while scores[0]<goal and scores[1]<goal:
        num_rolls = strategys[player](scores[player], scores[other(player)])
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(*scores):
        ...
        if gain > previous_high:
            if gain == 1:
                print("%d point! That's the biggest gain yet for Player %s" % (gain, who))
            else:
                print("%d points! That's the biggest gain yet for Player %s" % (gain, who))
            return announce_highest(who, gain, scores[who])
        else:
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    return max(range(1, 11), key=lambda i:average_roll_dice(i, dice))
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if opponent_score % bacon_score == 0 and opponent_score // bacon_score > 1:
        return 0
