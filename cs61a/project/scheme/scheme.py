...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lambda_expressions = Pair(expressions.first.second,
                                  expressions.second)
        target = target.first
        lambda_procedure = do_lambda_form(lambda_expressions, env)
        env.define(target, lambda_procedure)
        return target
        # END PROBLEM 10
    else:
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        raise SchemeError("Invalid macro: {}".format(expressions))

    # END Problem 21

def do_when_form(vals, env):
    return do_if_form(Pair(vals.first, Pair(Pair('begin', vals.second), Pair(vals.first, nil))), env)

def do_until_form(vals, env):
    new_expr = Pair(Pair('not', Pair(vals.first, nil)), vals.second)
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a dynamically-scoped call evaluated in call ENV."""
