...
class Server:
    ...
    def start(self):
        while True:
            ...
            data = conn.recv(ROOM_LENGTH)
            room = int(data.decode().strip())
            self.clients[room].append(conn)
            print(addr[0] + " connected to room: " + str(room))
    ...
    def receive_message(self, conn, addr):
        ...
        if len(message_header):
            room = int(conn.recv(ROOM_LENGTH).decode().strip())
            ...
            print("Room [{}] <{}>: {}".format(room, addr[0], message_data))
            return room, message_data
        else:
            return None
    ...
    def broadcast(self, message, connection, room):
        for client in self.clients.get(room, None):
            if client != connection:
                self.send_message(client, message)

    def remove(self, connection, room):
        clients = self.clients.get(room, None)
        if clients is not None and connection in self.clients:
