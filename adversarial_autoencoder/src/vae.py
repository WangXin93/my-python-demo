...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        kwouts = {'mu':mu, 'log_var':log_var}
        return x_reconst, kwouts

    def compute_loss(self, x):
        x_reconst, kwouts = self(x)
        mu, log_var = kwouts['mu'], kwouts['log_var']
...
model = VAE(img_size=img_size, h_dim=h_dim, z_dim=z_dim).to(device)
...
visualize_z(zs, ts, figsize=(8, 8), fname="vae_tsne.png")
