...
def load_data(img_fname, feat_fname):
    """Load images and features from pickled file.
    """
    img = pickle.load(open(img_fname, 'rb'))
    feat = pickle.load(open(feat_fname, 'rb'))
    return img, feat
...
def image_scatter(features, images, img_res, res=4000, cval=1.):
    ...
    f2d = TSNE(n_components=2).fit_transform(features)
    ...
    canvas = np.ones((np.ceil(res_x+max_width).astype(int),
                      np.ceil(res_y+max_height).astype(int),
                      3))*cval
...
def save_img(data, fname, cmap=None):
...
if __name__ == "__main__":
    img, feat = load_data(
        img_fname="images_hist_shape_cate.pkl",
        feat_fname="output_hist_shape_cate.pkl"
    )
    canvas = image_scatter(feat, img, 224, res=5500)
    save_img(canvas, 'accessory_shape_cate.png')
    print('done.')
